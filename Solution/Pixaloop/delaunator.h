...
namespace delaunator {
	...
	struct compare {
		...
		std::vector<vertex2D> const& coords;
		float cx;
		float cy;

		bool operator()(std::size_t i, std::size_t j) {
			const float d1 = dist(coords[i].x, coords[i].y, cx, cy);
			const float d2 = dist(coords[j].x, coords[j].y, cx, cy);
			const float diff1 = d1 - d2;
			const float diff2 = coords[i].x - coords[j].x;
			const float diff3 = coords[i].y - coords[j].y;
	...
	public:
		...
		Delaunator(std::vector<vertex2D> const& in_coords);
	...
	Delaunator::Delaunator(std::vector<vertex2D> const& in_coords)
		: coords(in_coords),
		triangles(),
		halfedges(),
		hull_prev(),
		...
		for (std::size_t i = 0; i < n; i++) {
			const float d = dist(cx, cy, coords[i].x, coords[i].y);
			if (d < min_dist) {
				i0 = i;
				min_dist = d;
			}
		...
		for (std::size_t i = 0; i < n; i++) {
			if (i == i0) continue;
			const float d = dist(i0x, i0y, coords[i].x, coords[i].y);
			if (d < min_dist && d > 0.0) {
				i1 = i;
				min_dist = d;
			}
		...
		for (std::size_t i = 0; i < n; i++) {
			...
			const float r = circumradius(
				i0x, i0y, i1x, i1y, coords[i].x, coords[i].y);
		...
		m_center_x = center.first;
		m_center_y = center.second;
		...
		for (std::size_t k = 0; k < n; k++) {
			...
			while (q = hull_next[e], !orient(x, y, coords[e].x, coords[e].y, coords[q].x, coords[q].y)) { //TODO: does it works in a same way as in JS
				e = q;
				if (e == start) {
					e = INVALID_INDEX;
					break;
